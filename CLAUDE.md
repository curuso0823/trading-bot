...
## 🟥 現況真相（2026-06-18，R0–R6 重建收束 + E1+E2 whipsaw 修正落地後）— 不可被舊污染結論帶偏
...
- 台股 paper-trading bot；**live＝benchmark 被動**（`config/settings.yaml strategy.mode=benchmark`＝0050 vol-target + **MA200-85% overlay；2026-06-18 起加 E1+E2 whipsaw 修正＝連 3 日跌破 MA200×0.99 才砍 15%、連 3 日站回 ×1.01 才回滿，config `regime_confirm_days=3`/`regime_band_pct=0.01`，引擎 additive 行為中性**；`src/strategy_engines/benchmark_engine.py` + `main.py` benchmark 任務）。`config/strategy.yaml` 現為**研究回測**配置（舊 active 參數；capped_sim/signal_builder/tech_signal 讀取），不再被 live 執行。研究回測引擎＝`src/backtest/capped_sim.py`（`run_capped`）。
...
*建立 2026-06-16｜更新 2026-06-18（R0→R6 完成 + 倉庫清理 + **E1+E2 whipsaw 修正整併入 live**：MA200 每日規則→連 3 日確認 + 1% 緩衝帶[`regime_confirm_days=3`/`regime_band_pct=0.01`]，結構 Gate PASS / alpha 仍 FAIL[IRvs基準B 是 beta、IRvs0050≈0]、additive 行為中性、`pytest` 105 綠；前次 2026-06-17＝R6 被動落地 0050+MA200-85%、舊 active 執行碼刪除）｜更新者統整於 `docs/RESEARCH_JOURNEY.md` §(f) + `docs/E1_E2_WALKFORWARD.md`｜性質：standing 真相與紀律。*
